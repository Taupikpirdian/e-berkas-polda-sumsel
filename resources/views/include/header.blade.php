...
<div class="header hor-header">
    <div class="container">
        <div class="d-flex">
            ...
            <a class="header-brand1" href="{{URL::to('/dashboard')}}">
                <?php
                    $user = Auth::user();
                    if($user->hasRole('kepolisian')) {
                        $role = "KEPOLISIAN SUMATERA SELATAN";
                        $images = asset('images/icon/icon.png');
                    } else if ($user->hasRole('kejaksaan')) {
                        $role = "KEJAKSAAN SUMATERA SELATAN";
                        $images = asset('images/icon/icon-kejaksaan-lg.png');
                    } else if ($user->hasRole('operator-01') || $user->hasRole('operator-02') || $user->hasRole('operator-03') || $user->hasRole('operator-04') || $user->hasRole('operator-kasi-pidum') || $user->hasRole('operator-kasi-pidsus')) {
                        $role = "OPERATOR KEJAKSAAN SUMATERA SELATAN";
                        $images = asset('images/icon/icon-kejaksaan-lg.png');
                    } else if ($user->hasRole('admin-kejaksaan')) {
                        $role = "ADMIN KEJAKSAAN SUMATERA SELATAN";
                        $images = asset('images/icon/icon-kejaksaan-lg.png');
                    } else if ($user->hasRole('pengadilan')) {
                        $role = "PENGADILAN SUMATERA SELATAN";
                        $images = asset('images/icon/icon-pengadilan.png');
                    } else if ($user->hasRole('admin-lapas')) {
                        $role = "ADMIN LAPAS SUMATERA SELATAN";
                        $images = asset('images/icon/icon-kemenkumham-lg.png');
                    } else if ($user->hasRole('lapas')) {
                        $role = "LAPAS SUMATERA SELATAN";
                        $images = asset('images/icon/icon-kemenkumham-lg.png');
                    } else if ($user->hasRole('admin-master')) {
                        $role = "ADMIN MASTER";
                        // $images = asset('assets/images/brand/logo-3.png');
                        $images = asset('images/icon/icon.png');
                    } else {
                        $role = "ADMIN";
                        // $images = asset('assets/images/brand/logo-3.png');
                ...
                ?>
                <img src="{{$images}}" style="height:36px;" class="header-brand-img" alt="logo">
            </a>
            <div class="d-flex align-items-center mx-4">
                <div class="row">
                    ...
                    <div class="col-12" style="font-size: 10px">
                        @if(thisAkses()->count() > 0)
                        <span class="badge bg-info text-white">
                            @foreach(thisAkses() as $key=>$akses)
                                @if($akses)
                                    @if($akses->satker)
                                        {{ $akses->satker ? $akses->satker->name : '-' }} 
                                    @else
                                        -
                                    @endif
                                @else
                                    -
                                @endif
                                @if(thisAkses()->count() == $key + 1) @else , @endif
                            @endforeach
                        </span>
                        @else
                            @hasanyrole('kepolisian|kejaksaan|admin-kejaksaan|pengadilan')
            ...
            <div class="d-flex order-lg-2 ms-auto header-right-icons">
                ...
                <div class="dropdown d-none d-md-flex notifications">
                    <a class="nav-link icon" data-bs-toggle="dropdown"><i class="fe fe-bell"></i><span class=" " id="bell_beep"></span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow ">
                        <div class="drop-heading border-bottom">
                            <div class="d-flex">
                                ...
                                <div class="ms-auto">
                                    <span class="badge bg-success rounded-pill" id='amount_count'></span>
                                </div>
                            </div>
                        </div>
                        <div class="notifications-menu content__notifikasi">
                            {{-- from ajax --}}
                        </div>
                        <div class="dropdown-divider m-0"></div>
                        <a href="{{URL::to('/notification/list-data-view')}}" class="dropdown-item text-center p-3 text-muted">View all Notification</a>
                    </div>
                </div><!-- NOTIFICATIONS -->
    ...
    <div class="container">
        <div class="d-flex">
            ...
            <a class="header-brand" href="index.html">
                ...
                <img src="{{$images}}" class="header-brand-img desktop-logo mobile-light" alt="logo">
            </a>
