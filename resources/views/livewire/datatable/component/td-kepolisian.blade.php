...
<td class="text-nowrap align-middle text-center v_p16">
    ...
    @elseif(isset($dp->fileP16)) {{-- hanya bisa dilihat jaksa dan admin jaksa --}}
        <a type="button" class="dropdown-item">
            <i class="fa fa-check-circle-o color-green" aria-hidden="true" data-bs-placement="top" data-bs-toggle="tooltip" title="" data-bs-original-title="Sudah ada P16"></i> 
        </a>
    @else
...
<td class="text-nowrap align-middle text-center v_berkas">
    @if(isset($dp->fileP16) && !isset($dp->fileResumeBerkasPerkara) && $dp->status != 5)
        <a type="button" class="dropdown-item" wire:click="$emit('detailModal', {{ $dp->id }}, 'modal-berkas', '#modalBerkas_{{$dp->id}}')">
            <i class="fa fa-plus-circle color-yellow" aria-hidden="true" data-bs-placement="top" data-bs-toggle="tooltip" title="" data-bs-original-title="Upload Berkas"></i> 
        </a>
