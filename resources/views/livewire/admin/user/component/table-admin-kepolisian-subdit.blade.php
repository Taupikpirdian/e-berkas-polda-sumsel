...
<table class="table border-top table-bordered mb-0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center align-middle">{{ $user->penyidik ? $user->penyidik->nrp : '-' }}</td>
            <td class="text-center align-middle">
              @if($user->penyidik)
                @if($user->penyidik->subDit)
                  <span class="badge bg-success text-white">{{ $user->penyidik->subDit->name }}</span>
                @else
                -
                @endif
              @else
            ...
            <td class="text-center align-middle">
              @if(!$user->roles->isEmpty())
                <span class="badge bg-success text-white">{{ $user->roles->pluck('name')[0] }}</span>
              @else
