<form action="#" method="post" wire:submit.prevent="validateDitPolda">
    @csrf
    <div class="form-group" wire:ignore>
        <label class="form-label form-label-required">Jenis Akun</label>
        <select wire:model='subdit_id' id="subdit_id" class="form-control select2-show-search">
            <option value=""> Pilih Jenis Akun</option>
            @foreach ($direktorats as $dit)
            <option value="{{ $dit->id }}">{{ $dit->name }}</option>
            @endforeach
        </select>
    </div>
    @error('subdit_id')
